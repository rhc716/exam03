...
int	main(int argc, char **argv)
{
	...
	if (!(file = fopen(argv[1], "r")))
		return (ft_puterr("1 Error: Operation file corrupted\n"));
	if (fscanf(file, "%d %d %c", &bg_w, &bg_h, &bg_c) != 3)
		return (ft_puterr("2 Error: Operation file corrupted\n"));
	if (!(bg_w > 0 && bg_w <= 300 && bg_h > 0 && bg_h <= 300))
        return (ft_puterr("3 Error: Operation file corrupted\n"));
	image = (char *)malloc(sizeof(char) * (bg_w * bg_h));
	// 배경 채우기
	memset(image, bg_c, bg_w * bg_h);

	// 도형 채우기
	read = fscanf(file, "%c %f %f %f %f %c", &pix_type, &pix_x, &pix_y, &pix_w, &pix_h, &pix_c);
	while (read == 6)
	{
		// type과 가로 세로 에러
        ...
        {
            free(image);
            return (ft_puterr("4 Error: Operation file corrupted\n"));
        }
        y = 0;
        while (y < bg_h)
        {
    ...
    if (read != -1)
	{
		free(image);
		printf("read : %d\n", read);
		return (ft_puterr("5 Error: Operation file corrupted\n"));
	}
	// 그리기
	y = 0;
	while (y < bg_h)
